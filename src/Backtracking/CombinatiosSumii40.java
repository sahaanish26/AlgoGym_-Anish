...
public class CombinatiosSumii40 {
    ...
    public  static List<List<Integer>> combinationSum2(int[] candidates, int target) {
       ...
       // System.out.println(Arrays.toString(candidates));
        ...
        combinationSum2Helper(candidates,target,0,path,res,set);
    ...
    public  static void combinationSum2Helper(int[] candidates, int target,int index , List<Integer> path ,  List<List<Integer>> result, Set<List<Integer>> set) {
        ...
        combinationSum2Helper(candidates,target-currentNumber,index+1,path,result,set);
        //unchoose
    ...
    while(index<candidates.length-1 &&  candidates[index]==candidates[index+1]){
       ...
       // path.add(currentNumber);
        combinationSum2Helper(candidates,target,index+1,path,result,set);
        //unchoose
       // path.remove(path.size()-1);
