...
public class WordSearchii212 {
    ...
    public static  List<String> findWords(char[][] board, String[] words) {
        ...
        //create a trie from all unique word of the array
       ...
       // now start dfs from each position and find all possible word that is made
       // for each possible word at each level check if the word is a prefix in trie node
       // if not prune that dfs path.if the current word is a word match in the trie
       // then add it in the result
       int m = board.length;
       ...
       for (int i=0;i<board.length;i++){
        for (int j=0;j<board[0].length;j++){
           ...
           // by default it is false visited[i][j] =false;
            String word ="";
           
        
           dfsHelper( i,  j , board, trie.root,visited,word,result);
           ...
           List<String> response = new ArrayList<>(result);
        return response;
                  
               }
           
               private static void dfsHelper(int i, int j, char[][] board, TrieNode root, boolean[][] visited, String res, HashSet<String> result) {
               ...
               if(i<0||j<0||i>=board.length||j>=board[0].length){
                ...
                // doing the visited check here instead of four if conditions with each dfs call
                if(visited[i][j]== true){
                    return;
                }
               //means the prefix is not there in trie so no point in travering
               if(!root.neighbour.containsKey(board[i][j])){
                return;

               }
               // chose the current and add to chosen
               res = res + board[i][j];
               visited[i][j]=true;
               // if the chosen so far is a word in trie then add it to the result
               if(root.neighbour.get(board[i][j]).endsHere){
                result.add(res);
               }

               // now go to all valid neighbours
               // we could have checked for visited here but to avoid if condition and redundancy of  same check 4 times
               dfsHelper(i+1,j,board,root.neighbour.get(board[i][j]),visited,res,result) ;
               dfsHelper(i,j+1,board,root.neighbour.get(board[i][j]),visited,res,result) ;
               dfsHelper(i,j-1,board,root.neighbour.get(board[i][j]),visited,res,result) ;
               dfsHelper(i-1,j,board,root.neighbour.get(board[i][j]),visited,res,result) ;

               visited[i][j]=false;
               // dont need to do unchoose of res since it is a string and not passed it is immutable
